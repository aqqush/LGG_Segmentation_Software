# -*- coding: utf-8 -*-
"""
Created on Mon Apr 20 14:21:34 2015
...
def AtlasBasedSegmentation(img1data, img2data, sliceZ, mask, mask3D, pmapWMSlice, pmapGMSlice, pmapCSFSlice):
...
#    maxZ = rect3D[1][2]
    ...
    (mIx,mIdy,mIz) = np.where(mask3D>0) 
    
    #3D mask data
    maskedDataImg1_3D = img1data[mIx,mIdy,mIz]  
    maskedDataImg2_3D = img2data[mIx,mIdy,mIz]
...
# This EM removes the unsupressed CSF in WM
    f = gmm.GMM(n_states=2, cvtype = 'full', n_iter = 10, thresh = 0.01)
...
#    probMask = g.predict_proba(maskedData.T)
    ...
    mask1Img[mIx,mIdy,mIz] = probMask[:,0]
    
    mask2Img = np.zeros((row,col,dep))
    mask2Img[mIx,mIdy,mIz] = probMask[:,1]
    
    
    mProb1data = np.multiply(mask1Img[indX3,indY3,sliceZ],img2data[indX3,indY3,sliceZ])
    mProb2data = np.multiply(mask2Img[indX3,indY3,sliceZ],img2data[indX3,indY3,sliceZ])
    
    
    mPb1 = np.mean(mProb1data)    
    mPb2 = np.mean(mProb2data)
